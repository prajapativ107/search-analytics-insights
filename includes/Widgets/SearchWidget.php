...
use SearchAnalyticsInsights\Ajax\SearchService;
...
final class SearchWidget extends \WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'search_analytics_insights_search_widget',
			__( 'Search Analytics Search Widget', Constants::TEXT_DOMAIN ),
			array(
				'description'                 => __( 'Displays a search icon that opens a search form.', Constants::TEXT_DOMAIN ),
	...
	public function widget( $args, $instance ): void {
		...
		$widget_id    = ! empty( $args['widget_id'] ) ? sanitize_html_class( (string) $args['widget_id'] ) : wp_unique_id( 'search-analytics-insights-search-widget-' );
		...
		>
			...
			>
				...
				<?php if ( $show_label ) : ?>
					<span class="search-analytics-insights-search-toggle-label"><?php echo esc_html__( 'Search', Constants::TEXT_DOMAIN ); ?></span>
				<?php else : ?>
					<span class="screen-reader-text"><?php echo esc_html__( 'Search', Constants::TEXT_DOMAIN ); ?></span>
				<?php endif; ?>
			</button>

			<div id="<?php echo esc_attr( $popup_id ); ?>" class="search-analytics-insights-search-popup" hidden>
				<div class="search-analytics-insights-search-panel">
					...
					<?php if ( $ajax_enabled ) : ?>
						...
						<div class="search-analytics-insights-search-results-wrap" hidden>
							<ul class="search-analytics-insights-search-results" role="listbox"></ul>
						</div>
					<?php endif; ?>
				</div>
			</div>
	...
	public function form( $instance ): void {
		...
		?>
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Widget Title', Constants::TEXT_DOMAIN ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'icon_size' ) ); ?>"><?php esc_html_e( 'Icon Size', Constants::TEXT_DOMAIN ); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'open_mode' ) ); ?>"><?php esc_html_e( 'Open Mode', Constants::TEXT_DOMAIN ); ?></label>
			<select class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'open_mode' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'open_mode' ) ); ?>">
				<option value="dropdown" <?php selected( 'dropdown', $instance['open_mode'] ); ?>><?php esc_html_e( 'Dropdown', Constants::TEXT_DOMAIN ); ?></option>
				<option value="modal" <?php selected( 'modal', $instance['open_mode'] ); ?>><?php esc_html_e( 'Modal Popup', Constants::TEXT_DOMAIN ); ?></option>
				<option value="slide-down" <?php selected( 'slide-down', $instance['open_mode'] ); ?>><?php esc_html_e( 'Slide Down', Constants::TEXT_DOMAIN ); ?></option>
			</select>
		</p>
		<p>
			...
			<label for="<?php echo esc_attr( $this->get_field_id( 'show_label' ) ); ?>"><?php esc_html_e( 'Show Label', Constants::TEXT_DOMAIN ); ?></label>
		</p>
		<?php
	}
	...
	private function get_toggle_label( string $title, bool $show_label ): string {
		if ( ! $show_label ) {
			return __( 'Open search', Constants::TEXT_DOMAIN );
		}

		if ( '' !== trim( $title ) ) {
			return sprintf(
				/* translators: %s: widget title */
				__( 'Open %s', Constants::TEXT_DOMAIN ),
				$title
			);
		}
	...
	public static function get_icon_markup(): string {
		return '<svg viewBox="0 0 24 24" aria-hidden="true" focusable="false" class="search-analytics-insights-search-icon" xmlns="http://www.w3.org/2000/svg"><path fill="currentColor" d="M10.5 4a6.5 6.5 0 1 0 4.02 11.62l4.43 4.43 1.41-1.41-4.43-4.43A6.5 6.5 0 0 0 10.5 4Zm0 2a4.5 4.5 0 1 1 0 9 4.5 4.5 0 0 1 0-9Z"/></svg>';
	}
}